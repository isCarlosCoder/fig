...
func TestMathSqrt(t *testing.T) {
	...
	if err != nil { t.Fatalf("runtime error: %v", err) }
	if out != "12" { t.Fatalf("expected '12', got %q", out) }
}
...
func TestMathConstants(t *testing.T) {
	...
	if len(lines) != 2 { t.Fatalf("expected 2 lines, got %d: %q", len(lines), out) }
	if !strings.HasPrefix(lines[0], "3.14159") { t.Fatalf("expected PI '3.14159...', got %q", lines[0]) }
	if !strings.HasPrefix(lines[1], "2.71828") { t.Fatalf("expected E '2.71828...', got %q", lines[1]) }
}
...
func TestMathInExpression(t *testing.T) {
	...
	out, err := runFig(t, src)
	if err != nil { t.Fatalf("runtime error: %v", err) }
	if out != "79" { t.Fatalf("expected '79', got %q", out) }
}
...
func TestMathWrongArgType(t *testing.T) {
	...
	_, err := runFig(t, src)
	if err == nil { t.Fatal("expected error for wrong arg type") }
}

func TestMathWrongArity(t *testing.T) {
	src := "use " + `"` + "math" + `"` + "; math.abs(1, 2);"
	_, err := runFig(t, src)
	if err == nil { t.Fatal("expected error for wrong arity") }
}
