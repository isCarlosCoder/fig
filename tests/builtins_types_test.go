...
if lines[i] != e { t.Fatalf("line %d: expected %q, got %q", i, e, lines[i]) }
}
}
...
if lines[1] != "42" { t.Fatalf("expected '42', got %q", lines[1]) }
if lines[2] != "1" { t.Fatalf("expected '1', got %q", lines[2]) }
}
...
if err != nil { t.Fatalf("runtime error: %v", err) }
lines := strings.Split(out, "\n")
if lines[0] != "3.14" { t.Fatalf("expected '3.14', got %q", lines[0]) }
if lines[1] != "0" { t.Fatalf("expected '0', got %q", lines[1]) }
}
...
if err != nil { t.Fatalf("runtime error: %v", err) }
lines := strings.Split(out, "\n")
if lines[0] != "123" { t.Fatalf("expected '123', got %q", lines[0]) }
if lines[1] != "true" { t.Fatalf("expected 'true', got %q", lines[1]) }
if lines[2] != "null" { t.Fatalf("expected 'null', got %q", lines[2]) }
}
...
_, err := runFig(t, src)
if err == nil { t.Fatalf("expected error for toInt with non-numeric string") }
}

func TestTypesToFloatBadString(t *testing.T) {
src := useTypes(`types.toFloat("xyz");`)
_, err := runFig(t, src)
if err == nil { t.Fatalf("expected error for toFloat with non-numeric string") }
}
...
lines := strings.Split(out, "\n")
if lines[0] != "true" { t.Fatalf("expected 'true', got %q", lines[0]) }
if lines[1] != "function" { t.Fatalf("expected 'function', got %q", lines[1]) }
}
