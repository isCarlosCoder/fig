...
func TestStringsJoin(t *testing.T) {
	...
	out, err := runFig(t, src)
	if err != nil { t.Fatalf("runtime error: %v", err) }
	if out != "a - b - c" { t.Fatalf("expected 'a - b - c', got %q", out) }
}
...
func TestStringsReplace(t *testing.T) {
	...
	out, err := runFig(t, src)
	if err != nil { t.Fatalf("runtime error: %v", err) }
	if out != "hello Fig" { t.Fatalf("expected 'hello Fig', got %q", out) }
}
...
func TestStringsRepeat(t *testing.T) {
	...
	out, err := runFig(t, src)
	if err != nil { t.Fatalf("runtime error: %v", err) }
	if out != "hahaha" { t.Fatalf("expected 'hahaha', got %q", out) }
}

func TestStringsCharAtOutOfBounds(t *testing.T) {
	src := useMath(`strings.charAt("hi", 5);`)
	_, err := runFig(t, src)
	if err == nil { t.Fatal("expected error for charAt out of bounds") }
}
