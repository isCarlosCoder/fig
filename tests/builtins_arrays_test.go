...
if out != "[1, 2, 3, 4]" { t.Fatalf("expected '[1, 2, 3, 4]', got %q", out) }
}
...
if lines[0] != "3" { t.Fatalf("expected '3', got %q", lines[0]) }
if lines[1] != "[1, 2]" { t.Fatalf("expected '[1, 2]', got %q", lines[1]) }
}
...
if lines[1] != "[2, 3]" { t.Fatalf("expected '[2, 3]', got %q", lines[1]) }
}
...
out, err := runFig(t, src)
if err != nil { t.Fatalf("runtime error: %v", err) }
if out != "[10, 20, 30]" { t.Fatalf("expected '[10, 20, 30]', got %q", out) }
}
...
if err != nil { t.Fatalf("runtime error: %v", err) }
lines := strings.Split(out, "\n")
if lines[0] != "20" { t.Fatalf("expected '20', got %q", lines[0]) }
if lines[1] != "[10, 30]" { t.Fatalf("expected '[10, 30]', got %q", lines[1]) }
}
...
out, err := runFig(t, src)
if err != nil { t.Fatalf("runtime error: %v", err) }
if out != "[2, 3, 4]" { t.Fatalf("expected '[2, 3, 4]', got %q", out) }
}
...
out, err := runFig(t, src)
if err != nil { t.Fatalf("runtime error: %v", err) }
if out != "[3, 2, 1]" { t.Fatalf("expected '[3, 2, 1]', got %q", out) }
}
...
out, err := runFig(t, src)
if err != nil { t.Fatalf("runtime error: %v", err) }
if out != "[1, 1, 3, 4, 5]" { t.Fatalf("expected '[1, 1, 3, 4, 5]', got %q", out) }
}
...
lines := strings.Split(out, "\n")
if lines[0] != "1" { t.Fatalf("expected '1', got %q", lines[0]) }
if lines[1] != "-1" { t.Fatalf("expected '-1', got %q", lines[1]) }
}
...
out, err := runFig(t, src)
if err != nil { t.Fatalf("runtime error: %v", err) }
if out != "3" { t.Fatalf("expected '3', got %q", out) }
}

func TestArraysPopEmpty(t *testing.T) {
src := useArrays(`let a = []; arrays.pop(a);`)
_, err := runFig(t, src)
if err == nil { t.Fatalf("expected error for pop on empty array") }
}

func TestArraysWrongType(t *testing.T) {
src := useArrays(`arrays.push("not_array", 1);`)
_, err := runFig(t, src)
if err == nil { t.Fatalf("expected error for non-array argument") }
}
