...
if out != `["a", "b"]` { t.Fatalf("expected keys, got %q", out) }
}
...
out, err := runFig(t, src)
if err != nil { t.Fatalf("runtime error: %v", err) }
if out != "[10, 20]" { t.Fatalf("expected values, got %q", out) }
}
...
out, err := runFig(t, src)
if err != nil { t.Fatalf("runtime error: %v", err) }
if out != "{x: 99, y: 2}" { t.Fatalf("expected merged object, got %q", out) }
}
...
if err != nil { t.Fatalf("runtime error: %v", err) }
lines := strings.Split(out, "\n")
if lines[0] != "{a: 1}" { t.Fatalf("expected original unchanged, got %q", lines[0]) }
if lines[1] != "{a: 999}" { t.Fatalf("expected clone changed, got %q", lines[1]) }
}
...
if err != nil { t.Fatalf("runtime error: %v", err) }
lines := strings.Split(out, "\n")
if lines[0] != "0" { t.Fatalf("expected '0', got %q", lines[0]) }
if lines[1] != "{}" { t.Fatalf("expected '{}', got %q", lines[1]) }
}

func TestObjectsWrongType(t *testing.T) {
src := useObjects(`objects.keys("not_object");`)
_, err := runFig(t, src)
if err == nil { t.Fatalf("expected error for non-object argument") }
}
