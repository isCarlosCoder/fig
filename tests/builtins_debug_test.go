...
func TestDebugInspectNumber(t *testing.T) {
	...
	if out != "<number: 42>" {
		t.Fatalf("expected '<number: 42>', got %q", out)
	}
}
...
func TestDebugInspectString(t *testing.T) {
	...
	if out != "<string: hi>" {
		t.Fatalf("expected '<string: hi>', got %q", out)
	}
}
...
func TestDebugInspectNull(t *testing.T) {
	...
	if out != "<null: null>" {
		t.Fatalf("expected '<null: null>', got %q", out)
	}
}
...
func TestDebugDumpArray(t *testing.T) {
	...
	if !strings.Contains(out, "array(2)") {
		t.Fatalf("expected array dump, got %q", out)
	}
	if !strings.Contains(out, "[0] number(1)") {
		t.Fatalf("expected element 0, got %q", out)
	}
}
...
func TestDebugDumpObject(t *testing.T) {
	...
	if !strings.Contains(out, "object(1)") {
		t.Fatalf("expected object dump, got %q", out)
	}
	if !strings.Contains(out, "a: number(1)") {
		t.Fatalf("expected key dump, got %q", out)
	}
}
...
func TestDebugType(t *testing.T) {
	...
	if out != "string" {
		t.Fatalf("expected 'string', got %q", out)
	}
}
...
func TestDebugAssertFail(t *testing.T) {
	...
	if err == nil {
		t.Fatalf("expected error for failed assert")
	}
}
